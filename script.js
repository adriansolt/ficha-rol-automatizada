var ambidiestria = this.getField("ambidiestria");
...
var armadura0_def = this.getField("armadura0_def");
var armadura0_calidad = this.getField("armadura0_calidad");
...
var puntos_base_restantes = this.getField("puntos_base");
...
var ventaja1_coste = this.getField("ventaja1_coste");
var ventaja2_coste = this.getField("ventaja2_coste");
var ventaja3_coste = this.getField("ventaja3_coste");
var ventaja4_coste = this.getField("ventaja4_coste");
var ventaja5_coste = this.getField("ventaja5_coste");
var ventaja6_coste = this.getField("ventaja6_coste");
var ventaja7_coste = this.getField("ventaja7_coste");
var desventaja1_coste = this.getField("desventaja1_coste");
var desventaja2_coste = this.getField("desventaja2_coste");
var desventaja3_coste = this.getField("desventaja3_coste");

var puntos_creacion = this.getField("puntos_creacion");
...
function ventaja_coste(event) {
  num = event.target.name.substr(7, 1);
  ventaja = this.getField("ventaja" + num).value;
  if (
    ...
    ventaja_lC = ventaja.toLowerCase();
    if (
...
function desventaja_coste(event) {
  num = event.target.name.substr(10, 1);
  desventaja = this.getField("desventaja" + num).value;
  if (
...
function main1() {
  ...
  ambidiestria.value = false;

  i = 0;
  j = 0;
  k = 0;

  ventaja_limite = false;

  // Ventajas
  i = 1;
  ventajas_list = [];
  while (i < 8) {
    ventaja = this.getField("ventaja" + i);
    if (ventaja.value) {
  ...
  while (j < lista_habilidades.length) {
    habilidad = lista_habilidades[j];
    esp_habilidad = this.getField("esp_" + habilidad);
    pd_habilidad = this.getField("pd_" + habilidad);
  ...
  puntos_gastados = 6 + Math.floor(Number(nivel.value) / 2);
  i = 0;
  while (i < lista_atr.length) {
    atr = lista_atr[i];
    atr_actual = this.getField(atr + "_actual");
    atr_base = this.getField(atr + "_base");
    atr_puntos = this.getField(atr + "_puntos");
    // atr_puntos = Number(atr_actual.value) - Number(atr_base.value);
  ...
  armadura0_def.value =
    Number(armadura0_calidad.value) * 10;

  // Categorias nivel
  puntos_base_restantes.value = 70 - puntos_base;

  // Nivel
  nivel_base = Number(nivel.value) == 0 ? 80 : 100;
  md_totales.value = Number(nivel.value) * 10 + nivel_base;


  var i_primera_ventaja;
  var i_ultima_ventaja;
  i = 0;
  while (i < ventajas_list.length) {
    ventaja = ventajas_list[i];
    var ventaja_formateada;
    var ventaja_nombre;
    if (ventaja.match("Apto en")) {
      ventaja_formateada = ventaja.substr(8).replace(/\./g, "").toLowerCase();
      // t manos ii
      ...
      if (ventaja_formateada.substr(ventaja_formateada.length - 2) === "ii") {
        ...
        // tmanos
        ventaja_field = this.getField("coste_" + ventaja_nombre);
        ventaja_field.value = 1;
      } else {
        // t manos
        // intelectuales
        ventaja_nombre = ventaja_formateada.replace(/\s+/g, "");
        i_primera_ventaja = null;
        i_ultima_ventaja = null;
        ...
        if (i_primera_ventaja !== null || i_ultima_ventaja !== null) {
          ...
          // tmanos
          coste_ventaja = this.getField("coste_" + ventaja_nombre);
          if (coste_ventaja) {
            coste_ventaja.value = Number(coste_ventaja.value) === 3 ? 2 : 1;
          }
        }
      ...
      if (i_primera_ventaja !== null || i_ultima_ventaja !== null) {
        // intelectuales
        incr = 1 * Number(nivel.value);
        if (ventaja_formateada.substr(ventaja_formateada.length - 2) === "ii") {
          incr = incr * 2;
        }
        while (i_primera_ventaja <= i_ultima_ventaja) {
          esp_ventaja = this.getField(
        ...
        // tmanos
        incr = 2 * Number(nivel.value);
        ventaja_nombre = ventaja_formateada.replace(/\s+/g, "");
        if (ventaja_formateada.substr(ventaja_formateada.length - 2) === "ii") {
          incr = incr * 2;
          ventaja_nombre = ventaja_nombre.substr(0, ventaja_nombre.length - 2);
        }
        esp_ventaja = this.getField("esp_" + ventaja_nombre);
        if (esp_ventaja) {
          esp_ventaja.value = Number(esp_ventaja.value) + incr;
        }
      }
  ...
  puntos_gastados =
    puntos_gastados -
    Number(ventaja1_coste.value) -
    Number(ventaja2_coste.value) -
    Number(ventaja3_coste.value) -
    Number(ventaja4_coste.value) -
    Number(ventaja5_coste.value) -
    Number(ventaja6_coste.value) -
    Number(ventaja7_coste.value) +
    Number(desventaja1_coste.value) +
    Number(desventaja2_coste.value) +
    Number(desventaja3_coste.value);

  puntos_creacion.value = Number(puntos_gastados);
  ...
  */
  ...
  mano_d_ocupada = false;
  mano_i_ocupada = false;
  manos_libres = true;
  i = 0;
}

function arma_check(event) {
    ...
    if (event.value !== "-") {
      ...
      if (arma_clase.value !== "Escudo") {
        ...
        while (j <= 7 && !encontrado) {
          tabla_arma = this.getField("tabla_arma" + j);
          tabla_arma_clase = this.getField("tabla_arma" + j + "_clase");
          if (
...
function habilidad_novel(event) {
...
function main2() {
  ...
  while (i < lista_habilidades.length) {
    var hab = lista_habilidades[i];
    var md_habilidad = this.getField("md_" + hab);
    var pd_habilidad = this.getField("pd_" + hab);
    var base_habilidad = this.getField("base_" + hab);
    pd_habilidad.value =
    ...
    if (Number(pd_habilidad.value) > 0) {
      base_habilidad.value = Math.floor(
        ...
        Number(this.getField("coste_" + hab).value)
      );
    ...
    var esp = this.getField("esp_" + hab);
    var cat_nivel = this.getField("cat_nivel_" + hab);
    var final_habilidad = this.getField("final_" + hab);
    if (ctm) {
