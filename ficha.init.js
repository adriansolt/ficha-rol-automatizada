...
function actualizarDatosCategoria(
    ...
    this.getField("cat_iniciativa").value = cat_iniciativa_v * Math.floor(nivel.value / 2);
    this.getField("coste_llA").value = coste_llA_v;
    this.getField("coste_parada").value = coste_parada_v;
    this.getField("cat_ataque").value = cat_ataque_v * Math.floor(nivel.value / 2);
    this.getField("cat_esquiva").value = cat_esquiva_v * Math.floor(nivel.value / 2);
    this.getField("cat_parada").value = cat_parada_v * Math.floor(nivel.value / 2);
        ...
        if (manos_ocupadas === false) {
            ...
            if (check.value === "Eq.") {
                ...
                if (arma_freq.value !== "—") {
                    ...
                    var fue_req_diff = Number(arma_freq.value) - Number(this.getField("fue_actual").value);
                    if (fue_req_diff > 0) {
...
base_llA.value = Math.floor(Number(this.getField("pd_llA").value) / Number(this.getField("coste_llA").value));
...
while (i < lista_habilidades.length) {
    ...
    pd_habilidad.value = Number(md_habilidad.value) * this.getField("valor_multiplo").value;
